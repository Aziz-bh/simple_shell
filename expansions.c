...
/**
 * expand_variables - Expand environment variables in the input line.
 * @data: A pointer to a struct containing program data.
 *
 * This function expands environment variables in the input line.
 * It replaces "$?" with the exit status of the previous command,
 * "$$" with the current process ID, and "$VAR" with the value of
...
void expand_variables(data_of_program *data)
{
    ...
    for (i = 0; line[i]; i++)
    {
        ...
        {
            // Skip "$" if followed by space or end of line
            continue;
        }
        else if (line[i] == '$')
        {
 ...
 * @data: A pointer to a struct containing program data.
 *
 * This function expands aliases in the input line. It replaces any
 * alias found in the input line with its corresponding value.
 */
